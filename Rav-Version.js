...
// =============================================
// MODALES CUSTOM — remplacent confirm/alert/prompt
// =============================================
...
// Modale saisie texte — remplace prompt()
...
async function saveMusicList() {
    await ipcRenderer.invoke('save-music-list',
        playlist.map(m => ({ name: m.name, path: m.path }))
    );
}
...
document.getElementById('removeMusicFolderBtn').addEventListener('click', async () => {
    const ok = await showConfirm(
        "Supprimer le dossier surveillé ? Les musiques resteront dans la bibliothèque.",
    ...
    // Vider complètement la bibliothèque
    playlist = [];
    await saveMusicList();
    currentPage = 0;
    displayStoredFiles();
...
async function startWatchingFolder(folderPath) {
    ...
    // On ajoute les fichiers qui ne sont pas encore là
    const newFiles = files.filter(f => !playlist.find(m => m.name === f.name));
    if (newFiles.length > 0) await addFilesToPlaylist(newFiles);

    // NOTE : On ne met plus d'écouteurs 'on' ici pour éviter les doublons !
    // Ils sont gérés globalement en bas du fichier.
}
...
async function renamePlaylist(index) {
    ...
    savePlaylistsToLocal();
    displayPlaylist();
    // Mettre à jour le titre si on est dans la vue détail
...
async function deletePlaylist(index) {
    ...
    savePlaylistsToLocal();
    displayPlaylist();
    // Retourner à la liste si on était dans la vue détail
...
// 1. Activer le bouton "Modifier le dossier" (Paramètres)
...
// 2. Écouter les NOUVELLES musiques ajoutées EN DIRECT
ipcRenderer.on('new-music-detected', async (event, file) => {
    // Si la musique n'est pas déjà dans notre tableau
    if (!playlist.find(m => m.name === file.name)) {
        const url = `file:///${file.path.replace(/\\/g, '/')}`;
        playlist.push({ name: file.name, url: url, path: file.path });
        
        // On trie par ordre alphabétique pour que ce soit propre
        playlist.sort((a, b) => a.name.localeCompare(b.name, 'fr', { sensitivity: 'base' }));
        
        await saveMusicList();
        displayStoredFiles(); // Mise à jour de l'écran
    }
});

// 3. Écouter les musiques SUPPRIMÉES ou RENOMMÉES EN DIRECT
